...
byte-identical) opened as **#128** — **completes LTV-Pn.4**. `LTV-Po` split into Po.1 (narrative-wiring) + Po.2
(recipe + e2e). `LTV-Po.1` (build_customer_population consumes narrative
...
per-tier `difficulty_profiles.yaml`; registry auto-discovers it; recipe-driven
difficulty resolution factored into shared `core/difficulty.py:
resolve_difficulty_params` (both schemes delegate; unified "no profiles" policy)
and `build_world` carries the resolved params on `spec.config` so snapshot
