...
class CompanySpec:
    ...
    def from_dict(cls, data: dict[str, Any]) -> CompanySpec:
        ...
        if not (
            isinstance(er, (list, tuple))  # noqa: UP038
            and len(er) == 2
...
class ProductSpec:
    ...
    def from_dict(cls, data: dict[str, Any]) -> ProductSpec:
        ...
        if not (
            isinstance(acv, (list, tuple))  # noqa: UP038
            and len(acv) == 2
        ...
        if not isinstance(terms, (list, tuple)) or not all(  # noqa: UP038
...
class MarketSpec:
    ...
    def from_dict(cls, data: dict[str, Any]) -> MarketSpec:
        ...
        if not isinstance(industries, (list, tuple)):  # noqa: UP038
            raise InvalidRecipeError(
        ...
        if not isinstance(geographies, (list, tuple)):  # noqa: UP038
            raise InvalidRecipeError(
...
class GtmMotionSpec:
    ...
    def from_dict(cls, data: dict[str, Any]) -> GtmMotionSpec:
        ...
        if not isinstance(channels, (list, tuple)) or not all(isinstance(c, str) for c in channels):  # noqa: UP038
            raise InvalidRecipeError(
...
class PersonaSpec:
    ...
    def from_dict(cls, data: dict[str, Any]) -> PersonaSpec:
        ...
        if not (
            isinstance(title_variants, (list, tuple))  # noqa: UP038
