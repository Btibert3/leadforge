...
# NOTE: build-level determinism (motif + subscriptions) is covered by
# tests/schemes/lifecycle/test_build_world.py; the stronger full-bundle
# byte-determinism smoke lives below in test_full_bundle_byte_deterministic.
...
def test_public_industry_region_features_have_variance(tmp_path) -> None:
    ...
    for col in ("industry", "region"):
        # Assert presence first: a dropped column must fail here, not vacuously pass.
        assert col in train.columns, f"{col} missing from public snapshot"
